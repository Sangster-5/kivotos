...
export async function POST(request: NextRequest) {
    ...
    await client.query("UPDATE admin SET verified_applications = array_append(verified_applications, $1) WHERE id = $2", [parseInt(data.applicationId), parseInt(decrypt(adminID as string))]);
    client.release();
