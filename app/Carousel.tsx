                ...
                <div className="relative w-full max-w-full mx-auto">
                    <div className="overflow-hidden" ref={emblaRef}>
                        <div className="flex gap-x-4">
                            {tasks.map((item) => (
                                <div id={item.id.toString()} key={item.id} className="relative flex flex-col flex-shrink-0 p-2 w-full md:w-1/3 bg-[#234154] text-white rounded-md">
                ...
                <><div className="relative w-full max-w-full mx-auto">
                    <div className="overflow-hidden" ref={emblaRef}>
                        <div className="flex gap-x-4 relative">
                            {requests.map((item) => (
                                <div id={item.id.toString()} key={item.id} className="flex flex-col flex-shrink-0 p-2 w-full md:w-1/3 bg-[#234154] text-white rounded-md gap-y-1">
                                    <h1 id='tenant-name' className="font-bold text-lg">{item.tenant_name}</h1>
                                    <h1 id='unit'>Unit: {item.unit}</h1>
                                    <p id='description'>{item.description}</p>
                                    <p id='property'>{item.property == "theArborVictorianLiving" ? "Arbor Victorian Living" : "Vitalia Courtyard"}</p>
                                    <h1 id='date'>{new Date(item.date_time).toLocaleString()}</h1>

                                    {!nonAdmin && (
                                        <div className="flex-grow flex flex-col justify-end p-1 gap-y-2">
                                            {!item.is_task && <button className='bg-[#192F3D] w-full h-8 text-white font-medium rounded-md mt-2' id={item.id.toString()} onClick={handleMaintenanceRequestToTask}>Create Task</button>}
                                            <select className='w-full drop-shadow-xl border-b-2 py-1 text-center rounded-md text-black' onChange={taskSelectCallback} defaultValue={item.status}>
                                                <option value="todo">Todo</option>
                                                <option value="in-progress">In Progress</option>
                                                <option value="completed">Completed</option>
                                            </select>
            ...
            {complaints && complaints.length != 0 && (
                <><div className="relative w-full max-w-full mx-auto">
                    <div className="overflow-hidden" ref={emblaRef}>
                        <div className="flex gap-x-4">
                            {complaints.map((item) => (
                                <div id={item.id.toString()} key={item.id} className="relative grid flex-shrink-0 p-3 w-full md:w-1/3 bg-[#234154] text-white rounded-md gap-y-1">
